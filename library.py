...
def load_library():
    try:
        with open(datalibrary_path,"r") as library:
            ...
            # Reverts dictionary back to object
            return [Song(**item) for item in songlib]
...
def add_song():
    ...
    song_to_add = Song(song_title,artist,genre,duration,bpm)
    songlibrary.append(song_to_add)
    with open(datalibrary_path,"w") as library:
        # converts object into dictionary
        json.dump([asdict(s) for s in songlibrary], library, indent = 2)
